...
board::board(){
    ...
    /*test: available Locations method
        ...
        if(piecesMap[desiredLocation] == '?'){
            ...
            if(myTurn(lastPlayed, piece)){
                    ...
                    if(piecesOnTheBoard == 9){
                        ...
                        //update the board for the last piece{TO-DO}
                    ...
                    if(piece == 'X' || piece == 'x')
                        {
                            xCount++; 
                            //cout<<"DEBUG(X)========piece="<<piece<<endl;
                        }
                    else if(piece == 'O' || piece == 'o' || piece == '0') //accidentally enters a 0? no problem //remove the else
                        {
                            oCount++;
                           // cout<<"DEBUG(O)========piece="<<piece<<endl;
                        }
...
//getters
map<string,char> board::getPiecesMap(){
    return piecesMap;
}
int board::getOCount(){
    return oCount;
